...
set(NMSIS_DSP_DIR "${MAIN_LIB_DIR}/main/NMSIS/DSP")
set(NMSIS_DSP_INCLUDE_DIR "${NMSIS_DSP_DIR}/Include")
...
set(NMSIS_DSP_SRC
    ...
    TransformFunctions/riscv_rfft_fast_f32.c
    TransformFunctions/riscv_cfft_f32.c
    TransformFunctions/riscv_rfft_fast_init_f32.c
    TransformFunctions/riscv_cfft_radix8_f32.c
    TransformFunctions/riscv_bitreversal2.c
    CommonTables/riscv_common_tables.c
    ComplexMathFunctions/riscv_cmplx_mag_f32.c
    StatisticsFunctions/riscv_max_f32.c
    StatisticsFunctions/riscv_rms_f32.c
    StatisticsFunctions/riscv_std_f32.c
    StatisticsFunctions/riscv_mean_f32.c
)
list(TRANSFORM NMSIS_DSP_SRC PREPEND "${NMSIS_DSP_DIR}/Source/")

set(CH32_STARTUP_DIR "${MAIN_SRC_DIR}/startup")
...
# XXX: This code is not CH32 specific
main_sources(CH32_ASYNCFATFS_SRC
    io/asyncfatfs/asyncfatfs.c
    io/asyncfatfs/fat_standard.c
)

# main_sources(CH32_MSC_SRC
#     msc/usbd_storage.c
# )

# main_sources(CH32_MSC_FLASH_SRC
#     msc/usbd_storage_emfat.c
#     msc/emfat.c
#     msc/emfat_file.c
# )

# main_sources(CH32_MSC_SDCARD_SRC
#     msc/usbd_storage_sd_spi.c
# )

set(CH32_INCLUDE_DIRS
    "${NMSIS_DSP_INCLUDE_DIR}"
    "${MAIN_SRC_DIR}/target"
)
...
set(CH32_LINKER_DIR "${MAIN_SRC_DIR}/target/link")
...
function(get_ch32_flash_size out size)
    # D: 480, E: 960 KiB
    string(TOUPPER ${size} s)
    if(${s} STREQUAL "D")
        set(${out} 480 PARENT_SCOPE)
        return()
    endif()
    if(${s} STREQUAL "E")
        set(${out} 960 PARENT_SCOPE)
        return()
    endif()
endfunction()
...
function(add_hex_target name exe hex)
    add_custom_target(${name} ALL
        ...
        ${CMAKE_OBJCOPY} -Oihex --set-start 0x0 $<TARGET_FILE:${exe}> ${hex}
        BYPRODUCTS ${hex}
    )
endfunction()
...
function(set_linker_script target script)
    set(script_path ${CH32_LINKER_DIR}/${args_LINKER_SCRIPT}.ld)
...
function(target_ch32)
    if(NOT riscv-wch STREQUAL TOOLCHAIN)
        return()
    endif()
    ...
    set(target_sources ${CH32_STARTUP_DIR}/${args_STARTUP})
    ...
    add_ch32_executable(
        ...
        SOURCES ${target_sources} ${msc_sources} ${NMSIS_DSP_SRC} ${COMMON_SRC}
